...
public:
	...
	virtual ~ibyFile() {};
	inline ibyFile & operator>>(ibychar_t & rhs);
	inline ibyFile & operator>>(unsigned char & rhs);
	inline ibyFile & operator>>(ibylen_t & rhs);
	inline ibyFile & operator>>(ibystring_t & rhs);
	inline void GetLenString(ibystring_t & rhs);

	typedef unsigned int pos_type;
};

inline ibyFile & ibyFile::operator>>(ibychar_t & rhs) {
	rhs = get();
	return *this;
}

inline ibyFile & ibyFile::operator>>(unsigned char & rhs) {
	rhs = get();
	return *this;
}

inline ibyFile & ibyFile::operator>>(ibylen_t & rhs) {
	rhs = (get() << 8) + get();
	return *this;
}

inline ibyFile & ibyFile::operator>>(ibystring_t & rhs) {
	ibychar_t ch;
	rhs.erase();
...
inline void ibyFile::GetLenString(ibystring_t & rhs) 
{
	ibychar_t tmp[300];
	get(tmp, get()+1); 
	rhs = tmp;
